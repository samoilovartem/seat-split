...
path_to_save = 'accounts/outlook/'


# ================================= READING CSV FILE ======================================
path_to_read = '/Users/samoylovartem/Documents/Data migration/Accounts/Mateen Outlook (full original version).csv'
df = pd.read_csv(path_to_read, sep=',', keep_default_na=False)

# =============================== DELETING SOME COLUMNS ====================================
# df.drop(df[df['ID'] == ''].index, inplace=True)
df.drop(columns=['Card', 'Expiry', 'CVV', 'Address', 'City', 'State', 'Zip'],
        axis=1, inplace=True)
print(df.dtypes)
