...
def send_email_confirmation(request: Request, user: User):
    ...
    mail_subject = 'Activate your account in Ticket Hive'
    message = render_to_string(
        'emails/account_verification.html',
        {'email': user.email, 'link': get_confirmation_link(request, user)},
    )
    to_email = user.email
    email = EmailMessage(
        ...
        body=message,
        to=[to_email],
        from_email='Ticket Hive <asamoilov@lewanddowski.com>',
    )
