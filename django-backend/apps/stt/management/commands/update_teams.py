...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        with open('seatscouts.json', 'r') as file:
            ...
            for item in data:
                try:
                    team = Team.objects.get(league=item['league'], name=item['description'])
                    team.automatiq_credentials_website_id = item['id']
                    team.save()
                    self.stdout.write(self.style.SUCCESS(f'Successfully updated {team.name}'))
                except Team.DoesNotExist:
                    self.stdout.write(
                        self.style.WARNING(f'Team not found for {item["league"]}, {item["description"]}')
                    )
