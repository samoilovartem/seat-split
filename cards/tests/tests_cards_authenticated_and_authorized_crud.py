...
from cards.tests.settings import FULL_USER_DATA, CARDS_LIST_URL, CARDS_FULL_VALID_TEST_DATA, \
...
class CreateCardTest(CardsTest):
    ...
    def test_can_create_card(self):
        ...
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Cards.objects.count(), 2)
        self.assertEqual(response.data.get('account_assigned'),
    ...
    def test_can_create_card_with_existing_email_and_platform(self):
        ...
        self.assertIn(member='The fields account_assigned, platform must make a unique set.',
    ...
    def test_can_create_card_with_incorrect_card_number(self):
        ...
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='Card number must have 15 or 16 digits!',
                      container=response.data.get('card_number'))

        CARDS_FULL_VALID_TEST_DATA.update({'card_number': '12345678876543210'})
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='Ensure this field has no more than 16 characters.',
                      container=response.data.get('card_number'))
    ...
    def test_can_create_card_with_incorrect_expiration_date(self):
        ...
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='Expiration date must be in format MM/YY',
                      container=response.data.get('expiration_date'))

        CARDS_FULL_VALID_TEST_DATA.update({'expiration_date': '10/230'})
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='Ensure this field has no more than 5 characters.',
                      container=response.data.get('expiration_date'))
    ...
    def test_can_create_card_with_incorrect_cvv_number(self):
        ...
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='CVV number must have 3 or 4 digits!',
                      container=response.data.get('cvv_number'))

        CARDS_FULL_VALID_TEST_DATA.update({'cvv_number': '12000'})
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='Ensure this field has no more than 4 characters.',
                      container=response.data.get('cvv_number'))
    ...
    def test_can_create_card_with_empty_city(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='This field may not be blank.',
                      container=response.data.get('city'))
    ...
    def test_can_create_card_with_incorrect_state(self):
        ...
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='State must have 2 letters only and be in the following format: XX!',
                      container=response.data.get('state'))

        CARDS_FULL_VALID_TEST_DATA.update({'state': 'gggg'})
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='Ensure this field has no more than 2 characters.',
                      container=response.data.get('state'))
    ...
    def test_can_create_card_with_incorrect_zip_code(self):
        ...
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='ZIP code must have 5 digits!',
    ...
    def test_can_create_card_with_incorrect_in_tm(self):
        ...
        self.assertIn(member='Must be a valid boolean.',
                      container=response.data.get('in_tm'))
    ...
    def test_can_create_card_with_incorrect_is_deleted(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn(member='Must be a valid boolean.',
                      container=response.data.get('is_deleted'))
    ...
    def test_can_create_card_with_not_existing_fields_passed(self):
        ...
        response = self.client.post(path=CARDS_LIST_URL, data=CARDS_FULL_VALID_TEST_DATA_COPY)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class ReadCardTest(CardsTest):
    ...
    def test_can_read_card_detail(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Cards.objects.get(pk=1).account_assigned,
...
class UpdateCardTest(CardsTest):
    ...
    def test_can_update_all_card_fields(self):
        ...
        response = self.client.put(path=CARD_DETAIL_URL, data=CARDS_FULL_VALID_TEST_DATA_COPY)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Cards.objects.get(pk=1).account_assigned,
